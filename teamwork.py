    ...
    try:
        ...
        for search_user in users:
            ...
            if not db.is_exist_searchuser(vk_id):
                user_info = user.get_user_info(vk_id, fields=['first_name', 'last_name', 'city', 'sex', 'bdate'])
                if user_info:
                    ...
                    logger.info(f"Пользователь {vk_id} добавлен в SearchUser")
                else:
                    logger.error(f"Не удалось получить информацию о пользователе {vk_id}")
